    ...
    */
    ...
    describe('New Feed Selection', function(){
        ...
        var feedLen = allFeeds.length,
            count,
            feedContentArray = [];
            for(count = 0;count<feedLen;count++){
                (function(){
                  countC = count;  
                    beforeEach(function(done){
                            loadFeed(countC, function(){
                                done();
                            });             
                    });
                    feedContentArray.push($('.feed'));
                })();     
            }
        for(var i = 1;i<feedLen;i++){
                it('loadFeed is diff for each feed', function(done){ 
                    expect(feedContentArray[i]).not.toEqual(feedContentArray[i-1]);
                    done();
            });
        }        
